...
const TABLE_HEAD = [
  ...
  { id: "Radiologue", label: "Radiologue Contacter", alignRight: false },
...
function applySortFilter(array, comparator, query) {
  ...
  if (query) {
    return filter(array, (_user) => {
      let user =
        _user?.destinataire.first_name + " " + _user?.destinataire?.last_name;
...
export default function DicomSent() {
  ...
  useEffect(() => {
    axiosPrivate
      ...
      .then((res) => {
        setDicom(reverse(res.data.user.images_envoyes));
      })
      .catch((err) => {
        console.log(err);
      });
  ...
  const handleRedirect = (event, dicom) => {
    navigate(APP_ROUTES.DICOMS + "/" + dicom.id);
      ...
      <Container>
        ...
        >
          <Typography variant="h4" gutterBottom>
            Liste des fichiers DICOM que vous avez envoyé
          </Typography>
        </Stack>
                  ...
                  return (
                    ...
                    >
                      <TableCell padding="checkbox">
                        <Checkbox
                          onChange={(event) =>
                            handleClick(event, dicom.destinataire.firstname)
                          }
                        />
                      </TableCell>
                      <TableCell
                      ...
                      <TableCell align="left">
                        {dicom?.destinataire?.first_name +
                          " " +
                          dicom?.destinataire.last_name}
                      </TableCell>
